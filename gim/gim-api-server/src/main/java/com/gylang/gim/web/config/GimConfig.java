...
public class GimConfig {
    ...
    private List<AdminMessageEventListener<MessageWrap>> eventListenerList;
    ...
    public NettyConfiguration nettyConfiguration() throws IllegalAccessException {
        ...
        if (null != eventListenerList) {
            for (AdminMessageEventListener<MessageWrap> eventListener : eventListenerList) {

                for (String key : eventListener.bind()) {
                    String[] s = key.split("-");
                    socketManager.typeAndCmdBind(Integer.parseInt(s[0]), s[1], messageWrap -> eventListener.onEvent(key, messageWrap));
                }
            }
        }
