...
public class IMServer {
    ...
    public void init() throws InterruptedException {
        EventLoopGroup workerGroup = nettyConfig.workerGroup();
        EventLoopGroup bossGroup = nettyConfig.bossGroup();
        ...
        ChannelFuture channelFuture = serverBootstrap.bind((int) nettyConfig.getProperties(NettyConfigEnum.SOCKET_SERVER_PORT)).sync();
        // 监听服务启动
        channelFuture.syncUninterruptibly().channel().newSucceededFuture().addListener(f -> {
            log.info("==================================================");
            log.info("================netty启动成功=====================");
            log.info("==================端口:{}=======================", (Object) nettyConfig.getProperties(NettyConfigEnum.SOCKET_SERVER_PORT));
            log.info("==================================================");
        });
        // 监听服务关闭
